...
# === Автоматический анализ ===
if st.session_state['page'] == "Автоматический анализ":
    st.title("🔍 Автоматический анализ")

    with st.expander("ℹ️ Как пользоваться этим разделом"):
        st.write(
            "Нажмите кнопку ниже, чтобы автоматически очистить данные: "
            "сначала будут обработаны пропуски, затем выбросы. "
            "После этого данные будут готовы к дальнейшему анализу."
        )
            ...
            with st.spinner("Шаг 2/2: обработка выбросов…"):
                ...
                try:
                    ...
                    if len(outlier_log) == 0:
                        ...
                        # st.write(pd.DataFrame(outlier_log))
                except Exception as e:
